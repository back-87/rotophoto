    ...
    with Image.open(config.ACTIVE_PATH) as image_object:
        ...
        while True:
            ...
            if target_orientation == "PORTRAIT" and text_width > MAX_PORTRAIT_WIDTH:
                ...
                if (
                    font_size < 20
                ):  # Floor limit safety cap so text never becomes microscopic
                    break
            else:
        ...
        if target_orientation == "PORTRAIT":
            ...
            canvas_draw.text(
                (padding, padding), text_string, font=font, fill=(255, 255, 255, 255)
            )
            ...
            final_draw.text(
                (text_x, text_y), text_string, font=font, fill=(255, 255, 255, 255)
            )

        final_img = final_img.convert("RGB")
        final_img.save(
            os.path.join(config.ACTIVE_DIR, "temp_overlayed.jpg"),
            "JPEG",
            quality=95,
            exif=exif_data,
        )
        ...
        if not no_fade_next_photo_change:
            os.replace(
                os.path.join(config.ACTIVE_DIR, "temp_overlayed.jpg"),
                os.path.join(config.ACTIVE_PATH),
            )
        else:
...
def on_message(client, userdata, msg):
    ...
    if (
        msg.topic == "homeassistant/sensor/picframe_image/state"
        and no_fade_next_photo_change
    ):
        if no_fade_next_photo_change and PENDING_DESTINATION_PATH is not None:
            print(
                "🎯 Screen is confirmed PITCH BLACK. Launching physical rotation loop..."
            )
            ...
            display_local_image(
                PENDING_DESTINATION_PATH,
                PENDING_DESTINATION_ORIENTATION,
                LAST_RECEIVED_IMAGE_HASH,
            )
        ...
        msg.topic == "homeassistant/sensor/picframe_image/attributes"
    ):  # apply our own text overlay using Pillow so it's oriented correctly
        date_str = None
        location_str = None
        ...
        try:
            ...
            date_str = datetime.datetime.fromtimestamp(
                m_in["EXIF DateTimeOriginal"]
            ).strftime("%B %e, %Y")
            location_str = m_in["location"]
            overlay_text = ("" if location_str is None else f"{location_str}. ") + (
...
def display_local_image(path, orientation, post_req_hash):
    ...
    try:
        ...
        if (
            orientation.upper() != rotation.CURRENT_STATE
            and PENDING_DESTINATION_PATH is None
        ):
            ...
            os.replace(
                os.path.join(MODULE_DIR, "4kBlackCopy.jpg"),
                os.path.join(config.ACTIVE_PATH),
            )
...
def handle_photo_display_post(hash):
    ...
    return jsonify({"status": "success", "keys_processed": list(data.keys())}), 200
...
if __name__ == "__main__":
    # STEP A: Force the file refresher loop to run on a pristine, undisturbed bus
    from target import Pico_Src_Refresher

    Pico_Src_Refresher.deploy_pico_project()

    # STEP B: Now that the serial sync is safely finished, release the remaining system modules!
    # These imports will now spin up their respective threads and initialize the I2C bus safely.
    ...
    pir_thread = threading.Thread(
        target=pir_sleep.pir_monitor_loop,
        daemon=True,
        args=[
            rotation.req_rp1,
        ],
    ...
    button_control_thread = threading.Thread(
        target=button_control.handle_inputs, daemon=True
    )
